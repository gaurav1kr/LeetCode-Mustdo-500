...
##429 ****[Problem Link]https://leetcode.com/problems/prime-palindrome****
    ...
    int primePalindrome(int n) {
        ...
        for (int len = 1; len < 6; ++len) {
            for (int root = pow(10, len - 1); root < pow(10, len); ++root) {
                ...
                string right = string(left.rbegin() + 1, left.rend());
                int x = stoi(left + right);
                if (x >= n && isPrime(x)) return x;
            }
        }
        return -1;
...
##430 ****[Problem Link]https://leetcode.com/problems/delivering-boxes-from-storage-to-ports****
